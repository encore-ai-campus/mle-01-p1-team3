...
| 김동석 | [@seventy4-git](https://github.com/seventy4-git) | 키워드 그룹 비교 분석, RAG 체인, ChromaDB 스키마 문서화 · Text-to-SQL 진단, 평가셋 구축 |
| 이승재 | — | 인벤 질문 데이터 시각화, Top-K Retriever 구현 · DB 연결, 팁 게시판 청킹 · 검색 개선 |

### 회고

**김동석** : 인벤(INVEN) 게시판(2,970여개)을 크롤링했고, 수집된 데이터를 `정제 | 청킹 | 벡터DB구축`하고, `RAG chain` 구현과 `평가셋` 까지 짧은 기간 정신 없이 달려 왔지만, 운 좋게 실력 있는 팀원 두분을 만나 일정에 맞춰 성공적인 프로젝트 마무리를 할 수 있었습니다. 이번 프로젝트를 통해 경험한 기술을 온전히 제 것으로 만들기 위한 새로운 RAG 기반 시스템 구축을 구상해 볼 계획입니다.
**이승재** :
